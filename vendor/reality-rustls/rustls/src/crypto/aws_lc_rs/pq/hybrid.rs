...
impl ActiveKeyExchange for ActiveHybrid {
    ...
    fn complete_reality(&self, peer_pub_key: &[u8]) -> Result<SharedSecret, Error> {
        // REALITY authentication uses the server's X25519 public key (pbk), which is
        // always 32 bytes. When the TLS key share group is hybrid (eg X25519MLKEM768),
        // `peer_pub_key` is not a hybrid server share, so we must only complete the
        // classical component here.
        if peer_pub_key.len() == self.layout.classical_share_len {
            return self.classical.complete_reality(peer_pub_key);
        }
        ...
        let cl = self
            .classical
            .complete_reality(classical_share)?;
        let pq = self
            .post_quantum
            .complete_reality(post_quantum_share)?;
