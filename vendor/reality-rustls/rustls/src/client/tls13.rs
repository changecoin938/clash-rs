...
pub(super) fn initial_key_share(
    ...
    // REALITY servers forward our initial ClientHello to the configured `dest`.
    // Some modern targets prefer PQ/hybrid groups (e.g. X25519MLKEM768) and will
    // send a HelloRetryRequest if we only offer X25519 key share. Prefer hybrid
    // when REALITY is enabled so we avoid HRR and allow the server to mimic the
    // target handshake.
    let group = if config.reality.is_some() {
        config
            .provider
            .kx_groups
            .iter()
            .copied()
            .find(|g| {
                g.usable_for_version(ProtocolVersion::TLSv1_3)
                    && g.name() == NamedGroup::X25519MLKEM768
            })
            .or_else(|| {
                config
                    .provider
