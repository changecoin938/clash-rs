...
fn emit_client_hello_for_retry(
    ...
    if let Some(key_share) = &key_share {
            ...
            .unwrap_or_default()
        {
                    ...
                    })
            {
                ...
                // REALITY forwards our ClientHello to its `dest`. Some targets are
                // intolerant of unknown groups when they appear first in KeyShare,
                // so prefer sending the classical component first for compatibility.
        ...
        #[cfg(feature = "std")]
        if config.reality.is_some()
            && std::env::var_os("RUSTLS_REALITY_DEBUG").is_some()
        {
            std::eprintln!(
                "rustls-reality: key_share_entries={:?}",
                shares
                    .iter()
                    .map(|s| (s.group(), s.payload.0.len()))
                    .collect::<Vec<_>>(),
            );
        }
    ...
    let mut cipher_suites: Vec<_> = config
        ...
        .collect();
    // REALITY servers derive the AEAD used for session_id auth from client behavior:
    // Xray uses AES-GCM, and some server versions pick based on cipher suite ordering.
    // To maximize compatibility, ensure AES suites come first when REALITY is enabled.
    if config.reality.is_some() {
        cipher_suites.sort_by_key(|suite| {
            match suite.as_str() {
                Some(name) => {
                    let name = name.to_ascii_lowercase();
                    let is_aes = name.contains("aes");
                    let is_chacha = name.contains("chacha");
                    match (is_aes, is_chacha) {
                        (true, _) => 0u8,
                        (false, true) => 2u8,
                        _ => 1u8,
                    }
    ...
    let key_share = if let Some(reality) = &config.reality {
        let raw_session_id_slice = &mut raw[39..(39 + 32)];
        for value in raw_session_id_slice.iter_mut() {
            *value = 0;
        }
        let public_key: [u8; 32] =
            <[u8; 32]>::try_from(reality.reality_public_key.clone()).unwrap();
        let mut short_id = reality.reality_short_id.clone();

        if short_id.clone().len() < 8 {
            for _i in 0..8 - short_id.clone().len() {
                short_id.push(0u8);
            }
        }
        ...
        let secret_key = key_share.complete_reality(&public_key)?;
        ...
        let plain_text: &[u8] = &[
            reality.reality_version_x,
            reality.reality_version_y,
            reality.reality_version_z,
            0,
            time[0],
            time[1],
            time[2],
        ...
        // Xray (and current REALITY servers) use AES-GCM with a 32-byte key here.
        let key = Key::<Aes256Gcm>::from_slice(key);
        let cipher = Aes256Gcm::new(&key);
        let session_id = cipher.encrypt(&nonce, payload).unwrap();
        for i in 0..32 {
            raw[39 + i] = session_id[i];
        }
        // rustls tracks the ClientHello session_id in `input.session_id` and uses it
        // to validate HRR/SH legacy_session_id_echo. REALITY rewrites the session_id
        // in the encoded ClientHello, so we must also update `input.session_id` to
        // match the bytes we actually sent.
        let mut session_id_encoded = Vec::with_capacity(1 + 32);
        session_id_encoded.push(32);
        session_id_encoded.extend_from_slice(&session_id);
        input.session_id = SessionId::read_bytes(&session_id_encoded).unwrap();
