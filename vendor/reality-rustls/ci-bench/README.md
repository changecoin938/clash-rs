...
their stdio to each other for communication (_i.e. stdio acts as the transport layer).
...
- For printf debugging, you should use `eprintln!`, because child processes use stdio as the
  transport for the TLS connection (_i.e. if you print something to stdout, you won't even see it
...
and therefore makes for a very good metric for automatic PR checking (_i.e. the automatic check
